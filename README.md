...
모바일 중심의 행사 안내 및 숙소·2일차 배차 조회 페이지입니다. GitHub Pages에서 운영할 수 있습니다.

## 바로 확인하기

`index.html`을 브라우저에서 열고, 샘플 성함 `권한별`을 입력하면 전체 흐름을 확인할 수 있습니다.
...
시트 열은 `ID | 이름 | 지역 | 호실 | 룸메이트 | 2일차 배차` 순서입니다. 룸메이트가 여러 명이면 쉼표로 구분합니다. 배차 값은 `1호차`, `2호차`, `3호차`, `자차 이동`처럼 입력할 수 있으며, 비워두면 조회 화면에 `배차 미정`으로 표시됩니다.
